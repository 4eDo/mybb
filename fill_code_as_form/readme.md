...
Строка ` const NEED_HIDE_NAVLINKS = true;` отвечает за принудительное сокрытие строки навигации ("Форум", "Участники" и т.д.). По умолчанию строка скрывается. Для отключения сокрытия укажите `false`.
...
Маркеры `{{CURRENT_USER_ID}}` и `{{CURRENT_TOPIC_SRC}}` НЕ НУЖДАЮТСЯ В ОТДЕЛЬНЫХ ПОЛЯХ ФОРМЫ.
