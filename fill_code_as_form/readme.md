...
> Иные дополнения к оформлению (копирование в буфер примеров кода, дизайн с использованием форумных стилей) можно увидеть здесь: https://github.com/4eDo/mybb/blob/main/fill_code_as_form/%D0%92%D0%B0%D1%80%D0%B8%D0%B0%D0%BD%D1%82%D1%8B_%D0%BE%D1%84%D0%BE%D1%80%D0%BC%D0%BB%D0%B5%D0%BD%D0%B8%D1%8F.md
...
#### ШПАРГАЛКА по видимости шаблона
| Форумы в шаблоне | Текущий форум | Топики в шаблоне | Текущий топик | Видимость шаблона |
|----------------|--------------|-----------------|--------------|------------------|
| ***all*** | любое | all | любое | ***ВИДНО*** |
| ***all*** | любое | 1 3 5 | любое | ***ВИДНО*** |
| ***all*** | любое | пусто | любое | ***ВИДНО*** |
| ***2*** 4 6 | 2 | all | любое | ***ВИДНО*** |
| 2 4 6 | ***1*** | all | любое | ***НЕТ*** |
| ***2*** 4 6 | 2 | 1 3 5 | любое | ***ВИДНО*** |
| ***2*** 4 6 | 2 | пусто | любое | ***ВИДНО*** |
| ***пусто*** | любое | ***all*** | любое | ***НЕТ*** |
| пусто | любое | ***1*** 3 5 | 1 | ***ВИДНО*** |
| пусто | любое | 1 3 5 | ***2*** | ***НЕТ*** |
| ***пусто*** | любое | ***пусто*** | любое | ***НЕТ*** |
...
**"info"** - здесь можно указать дополнительную информацию, пояснения и т.п.
- Можно оформлять через html
- Доступны шаблоны html-элементов:
	- ```{{LINK_TEMPLATE}}``` -- ссылка
	- ```{{BR_TEMPLATE}}``` -- перенос строки
	- ```{{STRONG_TEMPLATE}}```-- жирный текст
	- ```{{ITALIC_TEMPLATE}}``` -- курсив
	- ```{{UNDERLINE_TEMPLATE}}``` -- подчёркнутый
 - Для добавления своего шаблона (НЕ html) можно использовать тег ```<code></code>```
...
```
**"wrapperBefore"** - начало "обёртки". Если пользователь как-то заполнил поле, указанный в данном параметре текст будет вставлен ДО значения. Можно использовать html.

**"wrapperAfter"** - конец "обёртки". Если пользователь как-то заполнил поле, указанный в данном параметре текст будет вставлен ПОСЛЕ значения. Можно использовать html.
